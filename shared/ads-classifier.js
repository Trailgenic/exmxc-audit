...
export async function classifyPostings(postings) {
  ...
  try {
    ...
    return classified.map(item => {
      const title = String(postings.find(p => String(p?.posting_id) === String(item?.posting_id))?.title || "");
      return {
        posting_id: String(item?.posting_id || ""),
        tier: clamp(Number(item?.tier || 1), 1, 5),
        mcp_signal: Boolean(item?.mcp_signal),
        deployment_signal: Boolean(item?.deployment_signal ?? textIncludesAny(title, DEPLOYMENT_SIGNALS)),
        exploration_signal: Boolean(item?.exploration_signal ?? textIncludesAny(title, EXPLORATION_SIGNALS)),
        leadership: Boolean(item?.leadership ?? textIncludesAny(title, LEADERSHIP_SIGNALS)),
