...
function buildIdentityContext(crawlData, baseCtx) {
  ...
  if (personBlocks.length > 0 && orgSchemaName) {
    // Very conservative: we only flag conflict if Person.name exactly equals some *different*
    // org name variant; in practice we treat almost everything as non-conflict.
    ...
    // If a person is literally named identical to org, that is *weird*;
    // we could treat as conflict. For now, we treat that as non-conflict too.
    if (hardConflict) {
  ...
  const rootsSet = new Set(
    [identityRoot, canonicalRoot, hostNorm].filter((v) => typeof v === "string" && v.length > 0)
  );
