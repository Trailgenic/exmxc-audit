// /lib/ontologyEngine.js — Ontology Engine v0.0.3 (Phase 3 + Phase 4)
...
//   notes: [],
//   identity: { ... },
//   surface: { ... }   // <- now uses deriveSurfaceMapping (always structured, never null)
// }

import fs from "fs";
import path from "path";
import { deriveSurfaceMapping } from "./surfaceMapping.js";
...
export function evaluateOntology(crawlData) {
  ...
  /* STEP 4 — Surface / Perimeter Context (delegated to surfaceMapping.js) */
  const surfaceCtx = deriveSurfaceMapping({
    crawlData,
    baseCtx,
    identityCtx,
  });

  /* For constraints, we merge everything into a single context */
  const ctx = {
    ...baseCtx,
    ...identityCtx,
    ...surfaceCtx,
  ...
  return {
    ...
    // Full surface mapping object — always structured, never null
    surface: surfaceCtx,
  };
}
