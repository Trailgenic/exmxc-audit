...
function evaluateSurfaceCohesion(crawlData, baseCtx, identityCtx) {
  ...
  // For now, we treat "single canonical root" as convergence.
  ...
  // If the entity has multiple surfaces, we expect at least some schema identity.
...
export function evaluateOntology(crawlData) {
  ...
  /* ---------------------------------------------
     STEP 4 — Surface / Perimeter Context (Phase 4)
  --------------------------------------------- */
  ...
  return {
    ...
    warnings,
    notes,
    // Phase 4 surface/perimeter fields are already
    // baked into ctx and thus available in the caller
  };
}
