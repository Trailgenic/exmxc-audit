...
function normalizedTier(tierPoints, tierMax) {
  if (!tierMax) return 0;
  return clamp(Math.round((tierPoints / tierMax) * 100), 0, 100);
}
...
function scoreEntity({ entity, surfaces = [], auditedUrl } = {}) {
  const breakdown = [];
  let total = 0;
  ...
  /* ---------- Canonical ---------- */
  const finalAuditedUrl = auditedUrl || entity.url || entity.finalUrl || entity.finalURL || "";
  const canonicalConsistent = canonicallyMatches(entity.canonicalHref, finalAuditedUrl);
  let canonPts = entity.canonicalHref
    ? canonicalConsistent ? SIGNAL_WEIGHTS.canonical : 1
    : 0;
  total += canonPts;
  breakdown.push({
    ...
    notes: canonPts === SIGNAL_WEIGHTS.canonical
      ? "Canonical consistent"
      : canonPts
        ? "Present but inconsistent"
        : "Missing",
    raw: { canonical: entity.canonicalHref || "", auditedUrl: finalAuditedUrl }
  });
  ...
  const tier1Points = socialPts + aiPts + infPts + latticePts + extPts;
  const tier1Max = SIGNAL_WEIGHTS.social + SIGNAL_WEIGHTS.aiFidelity + SIGNAL_WEIGHTS.inference + SIGNAL_WEIGHTS.lattice + SIGNAL_WEIGHTS.external;
  const tier2Points = schemaPts + orgPts + bcPts + authorPts;
  const tier2Max = SIGNAL_WEIGHTS.schema + SIGNAL_WEIGHTS.orgSchema + SIGNAL_WEIGHTS.breadcrumb + SIGNAL_WEIGHTS.author;
  const tier3Points = titlePts + metaPts + canonPts + brandPts;
  const tier3Max = SIGNAL_WEIGHTS.title + SIGNAL_WEIGHTS.meta + SIGNAL_WEIGHTS.canonical + SIGNAL_WEIGHTS.brand;
  ...
  return {
    ...
    breakdown,
    tierScores: {
      tier1: { label: "Entity comprehension & trust", normalized: normalizedTier(tier1Points, tier1Max) },
      tier2: { label: "Structural data fidelity", normalized: normalizedTier(tier2Points, tier2Max) },
      tier3: { label: "Page-level hygiene", normalized: normalizedTier(tier3Points, tier3Max) }
    }
  };
}
