// /lib/surfaceMapping.js — EEI v5.3 (Phase 4 Surface Mapping)
...
export function deriveSurfaceMapping({ crawlData, baseCtx, identityCtx }) {
  ...
  const hostname =
    baseCtx.hostname || crawlData?.hostname || (url ? new URL(url).hostname : null);
  ...
  // Identity collisions: roots that look like peers, not pure authorities
  const identityCollisionRoots = auxiliaryRoots.filter(
    (r) => !externalAuthorityRootSet.has(r)
  );

  /* ---------------------------------------------
     9) Multi-surface schema coherence & perimeter stability
  --------------------------------------------- */
