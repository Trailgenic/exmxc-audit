...
function referenceFixtureProbe(url) {
  if (process.env.MCP_REFERENCE_FIXTURE_MODE !== "1") return null;
  if (!String(url || "").startsWith("https://mcp.exmxc.ai/.well-known/")) return null;

  if (url.endsWith("/tool-registry.json")) {
    return {
      detected: true,
      status: 200,
      valid: true,
      data: { tools: REFERENCE_TOOL_IDS.map(id => ({ id, name: id, inputSchema: { type: "object" } })) },
      format: "json",
      contentType: "application/json",
      textSample: null,
      dataPreview: "reference fixture tool registry",
      fixture: true
    };
  }
  ...
  if (url.endsWith("/mcp.json")) {
    return {
      ...
      status: 200,
      valid: true,
      data: { endpoint: "https://mcp.exmxc.ai/mcp", transport: "streamable-http" },
      format: "json",
      contentType: "application/json",
      textSample: null,
      dataPreview: "reference fixture mcp manifest",
      fixture: true
    };
  }
...
export async function probeEndpoint(url, options = {}) {
  const fixture = referenceFixtureProbe(url);
  ...
  try {
    ...
    return {
      ...
      format,
      contentType,
      textSample: textSampleOf(resp.data),
